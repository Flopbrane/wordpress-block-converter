...
def _check_link_markers(load_file: str, issues: list[LintIssue]) -> None:
    for line_number, marker_text in _iter_line_markers(load_file, "[リンク:"):
        ...
        issues.append(
            LintIssue(
                ...
                line_number,
                "リンクマーカーの形式が崩れています。",
                "[リンク:表示文字|https://example.com/] の形にしてください。",
            )
        )


def _check_image_markers(load_file: str, issues: list[LintIssue]) -> None:
    for line_number, marker_text in _iter_line_markers(load_file, "[画像:"):
