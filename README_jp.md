...
普通の `.txt` は、空行で段落を分けるだけのシンプルな変換として扱います。変換前に、見出し、リスト、引用、コードブロック、表、リンク、画像などの目印を付けたい場合は、[HOW_TO_USE_FOR_PLAIN_TEXT.md](HOW_TO_USE_FOR_PLAIN_TEXT.md) の書き方を使ってください。

目印付きの平文は `.wp_txt` または `.wptxt` として保存します。`.txt` は意図的に「段落中心の平文」として扱います。
